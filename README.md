...
Read-Host "If present, the aktest resource group was deleted. Press <Enter> to create a new 'akstest' resource group."

az group create --name $env:RESOURCE_GROUP --location $env:CLUSTER_LOCATION

Read-Host "Press <Enter> to create an AKS cluster '$($env:CLUSTER_NAME)' in resource group '$($env:RESOURCE_GROUP)'."

az aks create -g $env:RESOURCE_GROUP -n $env:CLUSTER_NAME --enable-managed-identity

Read-Host "AKS cluster created. Press <Enter> to get credentials so kubectl will use the new cluster."
...
Write-Host "Will create role assignment in 30 seconds. There is a race condition if we try too soon."
Start-Sleep 30
...
if ($env:IDENTITY_ASSIGNMENT_ID -eq "" -or $env:IDENTITY_ASSIGNMENT_ID -eq $null) {
    Read-Host "Still couldn't get the assignment id. Wait some time and then press <Enter>."
    $env:IDENTITY_ASSIGNMENT_ID="$(az role assignment create --role Reader --assignee $($env:IDENTITY_CLIENT_ID) --scope /subscriptions/$($env:SUBSCRIPTION_ID)/resourceGroups/$($env:IDENTITY_RESOURCE_GROUP) --query id -otsv)"
}
...
Read-Host "Press <Enter> to deploy AzureIdentity to the cluster using the following:$([Environment]::NewLine)$cmd"
...
Read-Host "Press <Enter> to deploy AzureIdentityBinding to the cluster using the following:$([Environment]::NewLine)$cmd"
...
Read-Host "Press <Enter> to deploy identity demo pod to the cluster using the following:$([Environment]::NewLine)$cmd"

$cmd | kubectl apply -f -

Read-Host "Press <Enter> to show logs for the demo pod."
kubectl logs demo
```